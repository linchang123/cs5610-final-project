...
export default function AnswerGroup({question, setQuestion}:{question: any; setQuestion: (q: any) => void}) {
    ...
    const handleAnotherAnswerButtonClick = () => {
        const newAnswer = {id: uuidv4(), answer: ""};
        if (question.questionType === "True/False") {
            return;
        } else if (question.questionType === "Multiple Choice") {
            const updatedPossibleAnswers = [...question.possibleAnswers, newAnswer];
            const updatedChoices = [...question.choices,{...newAnswer, isCorrect: false}];
            setQuestion({...question, possibleAnswers: updatedPossibleAnswers, choices: updatedChoices});
        } else if (question.questionType === "Fill in the Blank") {
            setQuestion({...question, possibleAnswers: [...question.possibleAnswers, newAnswer],
